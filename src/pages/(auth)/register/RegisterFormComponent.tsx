...
const RegisterFormComponent = () => {
  ...
  const onSubmit = async (values: RegisterSchoolValidatorType) => {
    try {
      ...
      toast.success(
        "Register sekolah berhasil, anda akan diarahkan ke halaman login!",
        {
          duration: 3000,
        },
      );
  ...
  return (
    <>
      <form
        className="flex min-w-md flex-col gap-3"
      ...
      >
        ...
        <p className="text-center">
          Sudah punya akun ? <Link to={"/login"} className="hover:underline">Login Sekarang</Link>
        </p>
      </form>
    </>
  );
